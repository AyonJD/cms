...
import { getClientById, updateClientById } from '../../../../apis/client.api';
...
import {
  ...
  handleTotalPaid,
  handleTotalPayment
} from '../../../../utils/functions/paymentHandler.function';
...
function EditClient() {
  ...
  useEffect(() => {
    ...
    const updateServiceStates = async () => {
      ...
      selectedServices.forEach((service) => {
        ...
        serviceStates[`${service}PaymentAmount`] =
          serviceData.servicePaymentInfo;
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      input.serviceStates &&
        Object.keys(input.serviceStates).forEach((service) => {
      ...
      const updatedInput = {
        ...
        const res = await updateClientById(clientid, updatedInput, token);
      if (res.status === 200) {
                        ...
                        return (
                          ...
                          >
                            ...
                            <Grid item xs={12} sm={4}>
                              <TextField
                                disabled
                                name={`${key}PaymentAmount`}
                                value={
                                  input.serviceStates[`${key}PaymentAmount`]
                                }
                                fullWidth
                            ...
                            <Grid item xs={12} sm={4}>
                              <TextField
                                name={`${key}PaidAmount`}
                                value={input.serviceStates[`${key}PaidAmount`]}
                                onChange={(e) => handleChange(e, setInput, key)}
                                fullWidth
                                label="Paid Amount"
                                placeholder=""
                              />
                            ...
                            <Grid item xs={12} sm={4}>
                              <TextField
                                value={
                                  ...
                                  Number(
                                    input.serviceStates[`${key}PaidAmount`]
                                  )
                                }
                                fullWidth
                                disabled
